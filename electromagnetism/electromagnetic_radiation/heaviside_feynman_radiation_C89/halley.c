 ...
 *      This is done using Halley's method. Given a guess, we obtain an       *
 *      improved guess via:                                                   *
 *                                                                            *
 *                           f(t ) f'(t )                                     *
 *                              0      0                                      *
 ...
 *      so-called Halley iterate. This function evaluates this expression.    *
 *  Notes:                                                                    *
...
/*  Function for computing the Halley iterate for the retarded time.          */
double halley(double rho, double time, double guess)
{
    ...
    /*  Halley's method uses the derivative, so the cosine is needed too.     */
    const double cos_tr = cos(guess);

    /*  The first derivative needs sin(tr)^2, and the second derivative needs *
     *  the fourth power. Pre-compute these.                                  */
    const double sin_tr_sq = sin_tr * sin_tr;
    const double sin_tr_qr = sin_tr_sq * sin_tr_sq;

    /*  The square of the distance is also needed.                            */
    ...
    const double dist = sqrt(rho_sq + sin_tr_sq);
     ...
     *                         c                                              *
     *                                                                        *
     *  Halley's method needs f(t0), f'(t0), and f''(t0) where t0 is the      *
     *  guess. We use c = 1, for simplicity.                                  */
     ...
     *               \/        r                                              *
     *                                                                        *
     *  This is used in Halley's method. We use c = 1.                        */
    const double deriv = 1.0 + sin_tr * cos_tr / dist;

    /*  The second derivative can be obtained by differentiating the previous *
     *  formula. We get the following.                                        */
    const double deriv2_numer = rho_sq * (1.0 - 2.0 * sin_tr_sq) - sin_tr_qr;
    const double deriv2_denom = dist * dist * dist;
    const double second_deriv = deriv2_numer / deriv2_denom;

    /*  Compute the Halley perturbation factor.                               */
    const double numer = func * deriv;
    const double denom = deriv * deriv - 0.5 * func * second_deriv;
    const double factor = numer / denom;

    /*  Return the Halley iterate.                                            */
    return guess - factor;
}
/*  End of halley.                                                            */
